...
export const getUserAPI = (userId: number) => {
  return axios.get(`${BASE_URL}/users/${userId}`).then((response) => response.data);
};

// 회원가입 - 이메일 입력, 인증 코드 전송
export const sendJoinEmailAPI = (signup_email: string) => {
  return axios.post(`${BASE_URL}/users/email`, { signup_email }).then((response) => response.data);
};

// 회원가입 - 인증 코드 입력
export const sendCodeAPI = (code: string) => {
  return axios.post(`${BASE_URL}/users/code`, { code }).then((response) => response.data);
};

// 회원가입 - 비번 입력
export const sendJoinPassword = (data: { signup_email: string; password: string; ck_password: string }) => {
  return axios.post(`${BASE_URL}/users/password`, data).then((response) => response.data);
};

// 회원가입 - 프로필 입력
export const setProfileAPI = (data: { signup_email: string; name: string; gender: string }) => {
  return axios.post(`${BASE_URL}/users/profile`, data).then((response) => response.data);
};

// 로그인  - 이메일 입력
export const sendLoginEmailAPI = (login_email: string) => {
  return axios.post(`${BASE_URL}/users/login/email`, { login_email }).then((response) => response.data);
};

// 로그인  - 비번 입력
export const sendLoginPasswordAPI = (data: { login_email: string; password: string }) => {
  return axios.post(`${BASE_URL}/users/login/password`, data).then((response) => response.data);
};

// 로그아웃
export const logoutAPI = () => {
  return axios.get(`${BASE_URL}/users/logout`).then((response) => response.data);
};

// 로그인  - 카카오톡
export const kakaoLoginAPI = () => {
  return axios.get(`${BASE_URL}/users/login/kakao`).then((response) => response.data);
};
